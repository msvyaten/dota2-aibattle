...
(`seen=18-26`, `out-of-reach=18-25`); на SF варда нет.
...
- **~140 захардкоженных hp-порогов дублируют именованные константы**: `0.45`=`activeRecovery`
  (27 мест), `0.35`=`danger` (22), `0.55`=`softRecovery` (16), `0.30`=`critical` (14).
  ⚠️ Сверялось ЗНАЧЕНИЕ, не смысл — читать каждое место. Риск реальный: подвинешь
  `Policy.danger` — 22 литерала не поедут. Прецедент оплачен в `7e2b066`.
- **Три из четырёх веток атаки вышки затенены четвёртой** — `terminal` первой и с `range+180`
  против `range+60`, за три матча 130 против нуля; комментарий стоит в коде. **`cs-watchdog`**
  взводит троттл ДО действия (намеренно, `b59c4e4`). **`ERA_START`** в `binding.py` охватывает
  обе популяции героев — разделить на эры.
- ⛔ **`binding.py` неотвечаем на текущей серии ПО ПОСТРОЕНИЮ**: конфиги написаны тремя моделями
  из ОДНОГО текста, диалы стоят в 0.05-0.10 (`BOUND=0`, `NO VARIANCE=9` на SF-эре). Разброс
  диала даёт разный ВХОДНОЙ ТЕКСТ, а не другая модель. ⭐ Решение юзера: второй противоположный
  текст, либо переписать инструмент под «насколько расходятся прочтения». Вечно красным — худшее.
...
- **Какая нога `anti-idle` уводит бота ВПЕРЁД на низком HP.** `8926148548` t=266: `recover`
  выиграл со 102 и вернулся пустым, тик упал на `anti-idle:2`, бот шёл к врагу с 35% до 8%.
  Ноги теперь инструментированы (`lane`/`lowhp-back`/`push`/`combat`) — данные есть, ответа нет.
