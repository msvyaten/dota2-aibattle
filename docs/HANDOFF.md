...
**=== ФАЗА 1 — 5v5 SMOKE-ТЕСТ (дёшево, без нового кода) ===**
Цель: масштабируются ли уже доказанные диалы на команду + смотрибельно ли 5v5.
1. Поднять 5v5 бот-лобби (10 ботов OHA, наши скрипты). Подтвердить: стабильно, без крашей, матч
   доигрывается до стат-дампа.
2. Применить 6 доказанных диалов КОМАНДНО: одна команда = «агрессивный» конфиг, другая = «пассивный»,
   зеркальные композиции (для начала те же герои у всех).
3. Зафиксировать: отличается ли командное поведение (групповой урон по героям, ранние стычки, карта
   присутствия) — с цифрами/диагом. Это решает, есть ли смысл в Фазе 2.
ПОДВОДНЫЕ КАМНИ (рапортовать сразу): запуск кастом-скриптов в 5v5-лобби, кик/наполнение слотов,
стабильность 10 ботов. Если 5v5 не поднимается на наших скриптах — стоп, обсуждаем.
