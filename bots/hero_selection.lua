	...
	for _, i in pairs(GetTeamPlayers(GetTeam())) do
		if not oboselect and IsPlayerBot(i) and IsPlayerInHeroSelectionControl(i) and GetSelectedHeroName(i) == "" then
			if Utils.IsHumanPlayerInAnyTeam() then
				...
				-- AIBattle: prefer customized pick (sSelectList[1] = pos1/mid in 1v1 mode)
				local customPick = sSelectList[1]
				if customPick and X.CanPickHero(GetTeam(), customPick) then
					hero = customPick
				else
		...
		elseif oboselect and IsPlayerBot(i) and IsPlayerInHeroSelectionControl(i) and GetSelectedHeroName(i) == "" then
			-- AIBattle: use customized pick for all slots (not Techies) to avoid buggy hero scripts
			local pick = sSelectList[1]
			if not pick or not X.CanPickHero(GetTeam(), pick) then
				pick = X.GetRandomAvailableHero(GetTeam(), tSelectPoolList[2]) or sSelectList[2]
			end
			SelectHero(i, pick)
			if Utils.HasValue(WeakHeroes, pick) then
				WeakHeroCount[GetTeam()] = WeakHeroCount[GetTeam()] + 1
