...
- [ ] **★ Released-hold аудит (класс «позиционер отпускает тик на месте», юзер-вопрос 20.07):**
      ...
      первый кандидат).
- [ ] **★★ anti-idle = де-факто лейн-воркер (форензика 8905429441, Fable 20.07):** R
      `anti-idle-creep=169` + `anti-idle-combat=122` — в 60с-срезе anti-idle выигрывал 6×
      против creep-work 1×. Он атакует ЛЮБЫХ крипов (пуш вейва против last_hit_only) и
      бегает между целями = хроническое юзер-«туда-сюда» in-lane, НЕ считается jitter-метрикой.
      Лечение = П1-C idle-band: visual-hold владеет тихими тиками, anti-idle-creep гейтится
      cwp==last_hit_only (стоять между ласт-хитами — корректное поведение). НЕ точечно:
      правка меняет фарм-профиль brawler'а. Счётчики выведены в postmatch (видимость).
