...
- **`153ee96` владелец расходников:** приёмка это ВЫЧИТАНИЕ, смотреть в обе стороны — суммарно
  выпитое НЕ падает, `heal-skip-trip-committed` ловит кларити по дороге на фонтан. ⛔ Отказы
  вендору переписаны ДО матча: `vendor-heal-suppressed-<предмет>` по фронту, а не окна на 30с.
