...
X.ConsiderItemDesire["item_clarity"] = function( hItem )
	...
	local skipClarity, skipClarityDetail = AIBItemPolicy.SkipManaConsumableForFountainTrip(bot)
	if skipClarity then
		AIBStyle.Blocked(bot, "mana-clarity", "fountain_free_mana", skipClarityDetail or "", 8.0)
		return BOT_ACTION_DESIRE_NONE
	end
...
X.ConsiderItemDesire["item_enchanted_mango"] = function( hItem )
	...
	for i = 0, 5 do
		...
		if ab ~= nil and ab:GetLevel() > 0 and not ab:IsPassive() then
			...
			if cost > mana and mana + 75 >= cost and cd <= 1.0 then
				abilitySoon = true
				break
			end
		end
