...
1. **wave-watch hold** — НОВЫЙ idle-кандидат в mode_laning (score 10, band=idle; ниже
   ...
   до anti-idle.
   ⚠️ **ПЕРЕКРЫТО 29.08 — «без ордера» больше не так.** `1c458c5` дал кандидату действие:
   если ластхиты не растут 6с (или `lh==0` после 35-й секунды 4с), он идёт к слабейшему
   атакуемому крипу. В первой редакции шаг не был загейчен по добиваемости, и ветка
   зациклилась — шаг к здоровому крипу ластхита не даёт, условие простоя остаётся истинным,
   через 6с снова шаг: `wave-watch-step=107` против `wave-watch-atk=18` за матч `8972598364`,
   ~16 приказов в минуту, `yoyo` у Radiant 2→7, мотор отобран у позиционера. `d377da7` сузил:
   шаг только к УЖЕ добиваемому крипу (тогда прогулка кончается ударом и простой снимается
   сам), здоровый крип → падаем в старое удержание с `blocked=wave-watch reason=not_finishable`,
   один приказ в секунду (`Visual.waveWatchStepHoldSeconds`).
   ⭐ Урок мандата: кандидат, который «идёт делать работу» при простое, обязан вести к
   действию, СНИМАЮЩЕМУ условие простоя, иначе он лечит собственный симптом.
...
падает в разы; ⛔ `state-wave-watch` — ФАНТОМ, такого ключа код никогда не испускал
(`ctx.state` его не зовёт, там `Style.DiagRL(bot, "wave-watch", 5)`), приёмка была
неотвечаема; реальные ключи — `wave-watch`, `-work`, `-atk`, `-step`, `-no-wave`;
LH не хуже; глазом — бот стоит у волны между
...
промпт (`aggressive|default`) корректен.

⛔ **Диал `ability_aggro` ВЫВЕДЕН 28.08.** Он был интенсивностью внутри правила, но гейтил
...
Рейзит вендорный `BotLib/hero_nevermore.lua` через `ActionQueue_UseAbility` — диал не менял
ничего наблюдаемого, а путь жёг тики. **Диал надо ПЕРЕРАБОТАТЬ и вернуть**, а не считать
закрытым.

⚠️ **Следствие, которое надо держать в голове.** SF остался ЕДИНСТВЕННЫМ героем без
harass-записей — у остальных 14 они на месте. Значит на живом матчапе
`ability_usage="aggressive"` больше НЕ даёт раз-хараса. Правило не умерло (оно ещё гейтит
покупку манго, `aibattle_item_policy.lua:155`), но на SF теперь копит ману под харас, который
наш путь не выполнит. В промпте это оговорено явно.
