...
-- AIBattle: lazy-load per-team playstyle config
local _STYLE = nil
local function GetPlayStyle()
    if _STYLE ~= nil then return _STYLE end
    local b = GetBot()
    local fname = (b:GetTeam() == TEAM_RADIANT)
        and "Customize/playstyle_radiant"
        or  "Customize/playstyle_dire"
    local ok, s = pcall(require, fname)
    _STYLE = ok and s or {}
    return _STYLE
end
...
if local_mode_laning_generic or (J.GetPosition(bot) == 1 and J.IsPosxHuman(5)) then
	function Think()
		...
		-- AIBattle: style-driven harass and ability usage
		local style = GetPlayStyle()

		-- Shrapnel harassment (only if ability_aggro = true)
		if style.ability_aggro then
			local shrapnel = bot:GetAbilityByName("sniper_shrapnel")
			if shrapnel ~= nil and shrapnel:IsFullyCastable() then
		...
		-- Auto-attack harassment (only if farm_priority = false)
		if not style.farm_priority then
			local desire = style.harass_desire or 0.3
			local targets = bot:GetNearbyHeroes(bot:GetAttackRange() + 50, true, BOT_MODE_NONE)
			if #targets > 0 and math.random() < desire then
				bot:Action_AttackUnit(targets[1], true)
				return
			end
		end

		-- AIBattle: suppress forward positioning when tower_safe = true
