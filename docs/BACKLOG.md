...
- [ ] **★★ ФАРМЕР БЕЗ БУТЫЛКИ (n=2: 8903952032, 8905027149 — весь матч bottle_last=-1,
      lh коллапсит до 10-15)** = повтор watchlist#0 («бутылка в сташ, курьер не везёт»,
      гейт `ability_item_usage:720-724`; в 8903988046 при lh=66 бутылки тоже не было 16.5 мин
      → НЕ экономика). По правилу watchlist n≥2 → фикс обязателен: дизъюнкт «вези готовый
      не-компонент» (зона Codex/vendor, прецедент). ПЕРВЫЙ приоритет после П3-B.2 —
      резервный вариант: диаг-строка в courier-гейт для подтверждения сташ-гипотезы.
