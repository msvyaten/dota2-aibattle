...
- Claude changed the live bindings to `Radiant=canonical_brawler` and `Dire=canonical_farmer`, and repurposed `canonical_farmer` into a Brawler mirror. Deploy bug found: `tools/deploy.bat playstyle` copied only `canonical_pusher` and `canonical_ganker`, so live `canonical_farmer.lua` could stay stale while the binding pointed at it. The playstyle profile now copies all four canonical presets.
- Live `8868824111` on `3133d97` finally showed post-horn `laning-core`, but visible standing came from repeated `AIB ERR` crashes: `aibattle_utils.lua:85` used unavailable `GetAttackCapabilities()`, and `mode_laning_generic.lua:646` indexed `AIBLanePolicy.Siege` after the constants table had been overwritten by function `M.Siege(args)`. Fixed by using attack range for ranged checks and moving siege constants to `AIBLanePolicy.SiegeConfig`.
- Full `8868866298` on `d709fb6`: no ownership regression, Dire won 2-0 in 5:08, but `AIB ERR` still existed. `GetGroundHeight` is also unavailable in this runtime, so `UphillMiss` now uses `GetHeightLevel(location)` like existing project code. `aba_global_overrides.lua` also guarded `debug.traceback()` because `debug` can be nil in live. Do not judge top-arbiter `empty_action` until the next error-free match; many fight/safety gates depend on `UphillMiss`.
