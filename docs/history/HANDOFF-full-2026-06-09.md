...
> Последнее обновление: 2026-06-09 (phase-12: regen_lane + defensive_heal + heal-pullback фикс; SF 1v1 матчи 8845060220-8845199644; item decoder в match_stats; текущий тест §22).
...
**Накачено сегодня (09.06):**
| # | Задача | Статус |
|---|---|---|
| ✅ | `lane_activity` дал — бить крипов между last-hit'ами, не стоять АФК | DONE |
| ✅ | `stay_regen` low_hp_behavior — отойти 400 юн., ждать регена без TP | DONE |
| ✅ | Дал-таблица R vs D в `match_stats.py` | DONE |
| ✅ | `lane=` в cfg-announce, `lane-active`/`stay-regen` счётчики | DONE |

**Дальше (не горит):**
| # | Задача | Приоритет |
|---|---|---|
| 7 | **Поведение до выхода крипов** — отдельный модуль для -90..0:00 (не патч в laning). Активен пока `DotaTime() < 0`, после — дефолтное лейнинг+руны поведение. Новый рул `pregame_behavior`. **Особенности 1v1:** руны в 0:00 НЕ спавнятся; нейтралы почти не атакуют. Варианты: `safe_tower` (у своей вышки), `aggressive_mid` (позиция за рекой), `jungle_pressure` (лес врага). Реализация: отдельный блок или файл. | MEDIUM |
| 8 | **Lane equilibrium** — заморозка крипов / позиция на линии | LOW |
| 9 | **Kill timing** — агрессия при level 6 (Requiem открыт) | LOW |
| 10 | **5v5 полный матч** — Pusher vs Ganker до победы | MEDIUM |
| 11 | **LLM item_build для 5v5** — Pusher/Ganker составы | MEDIUM |
| 12 | **fight_back coherence rule** в system prompt | LOW |

---

## §22 — Сессия 09.06.2026 (phase-12: regen_lane + defensive_heal)

### 22.1 Контекст

1v1 SF зеркало. Фокус: подтвердить что `regen_lane` поведение работает (бот отходит
от крипов и ждёт регена на линии вместо TP на фонтан). Параллельно — тест item builds
(OHA дефолт vs Valve build из `itembuilds/default_nevermore.txt`).

**Матчи этой сессии:**

| ID | Radiant | Dire | Итог |
|---|---|---|---|
| 8845060220 | tp_fountain | regen_lane | R победил, матч короткий, Dire до лоу-хп не дошёл |
| 8845078226 | tp_fountain | regen_lane | regen-lane не сработал (баг) |
| 8845101274 | regen_lane | tp_fountain | regen-lane не сработал (баг) |
| 8845199644 | regen_lane | tp_fountain | heal-item ✅, prev-heal ✅; regen-lane ❌ (баг найден) |

### 22.2 Что реализовано (LIVE, не закоммичено)

#### `bots/mode_laning_generic.lua`

**1. GetDesire() — не отключаться при атаке для regen_lane**
```lua
if not J.WeAreStronger(bot, 1200) or (nDistFromLane > 700 and J.GetHP(bot) < 0.7) then
    if AIBStyle.Get().rules.low_hp_behavior ~= "regen_lane" then
        return BOT_MODE_DESIRE_NONE
    end
end
```
Без этого: laning возвращал DESIRE_NONE когда бот атакуется + враг рядом → regen_lane
блок никогда не достигался.

**2. heal-pullback — пропускать для regen_lane**
```lua
else
    -- regen_lane handles its own retreat logic — skip heal-pullback so it can fire
...
```
Без этого: `heal-pullback` срабатывал первым (строка 399) и делал `return` → regen_lane
блок (строка 412) никогда не достигался.

**3. regen_lane блок** (строки 412–438):
```lua
if Style.Get().rules.low_hp_behavior == "regen_lane"
    and J.GetHP(bot) < (0.30 + 0.15 * (dials.retreat_caution or 0.5)) then
    -- ~41% HP при retreat_caution=0.75
    ...
    if not recentHeroDmg and not enemyChasing then
        -- rate-limit 3с: шаг назад от центроида вражеских крипов (400 ед.)
        if bot.aib_regenLast == nil or DotaTime() - bot.aib_regenLast >= 3.0 then
            local cen = AIB_EnemyCreepCentroid(nEnemyCreeps)
            local back = cen and J.VectorAway(bot:GetLocation(), cen, 400)
                              or AIB_ForwardSurvivingTowerLoc()
            if back then
                bot.aib_regenLast = DotaTime()
                AIBStyle.Diag(bot, "regen-lane"); bot:Action_MoveToLocation(back); return
            end
        end
    else
        AIBStyle.DiagRL(bot, "retreat-blocked", 3.0)  -- враг мешает выходу
    end
end
```

**4. Превентивный хил (prev-heal)** — до defensive_heal, при HP < 75% И полностью безопасно
(no hero damage 3с, no enemy 900 ед., кулдаун 30с):
```lua
local PREV_HEAL_THRESHOLD = 0.30 + 0.20 * (dials.retreat_caution or 0.5) + 0.30  -- ~75%
if GetImp('defensive_heal')
    and J.GetHP(bot) < PREV_HEAL_THRESHOLD
    and not bot:WasRecentlyDamagedByAnyHero(3.0)
    and #(bot:GetNearbyHeroes(900, true, BOT_MODE_NONE)) == 0
    and (bot.aib_prevHealLast == nil or DotaTime() - bot.aib_prevHealLast >= 30.0) then
    -- tango, salve, bottle (в безопасности)
    -- диаг: "prev-heal"
end
```
Подтверждён в матче 8845199644: R#2, D#3.

**5. defensive_heal — фиксы:**
- Убран гейт `not lhSecurable` (блокировал хил когда в зоне атаки добиваемый крип)
- Добавлена поддержка tango: `bot:Action_UseAbilityOnTree(tango, trees[1])`

**6. HandleRespawn debug diags:**
```lua
local tp = bot:GetItemInSlot(bot:FindItemSlot("item_tpscroll"))
if tp == nil then AIBStyle.DiagRL(bot, "respawn-no-tp", 5); return false end
if not tp:IsFullyCastable() then AIBStyle.DiagRL(bot, "respawn-tp-cd", 5); return false end
```

#### `bots/mode_retreat_generic.lua`

**tp-fountain diag** — подтверждает активацию tp_fountain поведения (rate-limit 5с):
```lua
if res >= 0.5 and lhb == "tp_fountain" then
    AIBStyle.DiagRL(bot, "tp-fountain", 5)
end
```
Подтверждён: R#13-19 в нескольких матчах.

#### `tools/match_stats.py`

**Item ID decoder** — `ITEM_NAMES` (100+ ID) + `decode_items()`:
```python
ITEM_NAMES = { 1:"blink", 29:"boots", 41:"bottle", 44:"tango", 100:"cyclone",
               116:"bkb", 216:"mango", 237:"faerie_fire", ... }
def decode_items(id_str): ...
```
Вместо числовых ID в выводе — читаемые имена.

### 22.3 Баги исправлены (путь к regen-lane)

| Баг | Симптом | Причина | Фикс |
|---|---|---|---|
| regen-lane=0 (корень 1) | laning деактивируется под атакой | GetDesire() → DESIRE_NONE при атаке+враг рядом | исключение для regen_lane (§22.2 п.1) |
| regen-lane=0 (корень 2) | regen_lane блок не достигается | heal-pullback делал return раньше (строка 399 < строка 412) | skip heal-pullback для regen_lane (§22.2 п.2) |
| defensive_heal не срабатывал | heal-item=0 | `not lhSecurable` гейт + improvements не в конфиге | убран гейт; добавлен `defensive_heal=true` в оба конфига |
| tango не ел | — | item_tango не был в heal-списке | добавлен с UseAbilityOnTree |
| TP после смерти не работал | бот шёл пешком | TP scroll мог быть на КД | добавлены diags respawn-no-tp / respawn-tp-cd |

### 22.4 DiagRL — доступные ключи этой сессии

| Ключ | Где | Смысл |
|---|---|---|
| `regen-lane` | mode_laning | шаг назад для регена (безопасно, сработал) |
| `retreat-blocked` | mode_laning | хотел regen но враг рядом или недавний урон от героя |
| `heal-pullback` | mode_laning | отход к башне (НЕ regen_lane билд) |
| `heal-item` | mode_laning | instant-предмет использован (wand/stick/ff/satanic) |
| `prev-heal` | mode_laning | превентивный хил (tango/salve/bottle, полностью безопасно) |
| `tp-fountain` | mode_retreat | tp_fountain режим активирован |
| `respawn-no-tp` | mode_laning | умер без TP scroll → будет идти пешком |
| `respawn-tp-cd` | mode_laning | TP scroll на КД → будет идти пешком |

### 22.5 Item builds SF 1v1

| Сторона | Build | Откуда |
|---|---|---|
| regen_lane | Valve official (Eul's+Blink) | `itembuilds/default_nevermore.txt` |
| tp_fountain (контроль) | OHA hero_nevermore.lua pos_2 | нет `item_build` в конфиге → OHA дефолт |

**Valve build:** slippers+tango+mango×3+circlet → bottle+magic_stick+boots →
magic_wand+cyclone+travel_boots+blink → BKB → arcane_blink+abyssal+shard → ...

**OHA build (pos_2):** tango+branches×2+ff+mango×2 → bottle+magic_wand+power_treads →
dragon_lance+crystalys+BKB+orchid → ...

### 22.6 Текущий тест (LIVE)

| Сторона | `low_hp_behavior` | item_build | `defensive_heal` |
|---|---|---|---|
| **Radiant** | `regen_lane` | Valve (Eul+Blink) | true |
| **Dire** | `tp_fountain` (контроль) | OHA дефолт | true |

Оба: `skill_build` = {1,5,1,5,1,6,1,5,5,4,6,4,4,4,6}, все диалы нейтральны (0.50–0.80).

### 22.7 Что ждём в следующем матче

- `regen-lane` — должен появиться у **Radiant** (R#N > 0)
- `retreat-blocked` — появится если враг мешал выходу
- `heal-pullback` — только у **Dire** (D#N); у Radiant не должно быть
- `tp-fountain` — только у **Dire** (D#N); у Radiant не должно быть

### 22.8 Открытые задачи (phase-12)

| # | Задача | Приоритет |
|---|---|---|
| 0 | **Запустить матч с новой парой** → подтвердить regen-lane R#N | NEXT |
| 1 | **kill-priority attack** — когда HP врага < execute_threshold, всегда атаковать героя (no roll). Блок перед harass. Диаг: `kill-priority`. | MEDIUM |
| 2 | **Respawn TP** — подтвердить respawn-no-tp / respawn-tp-cd в следующем матче | NEXT |
| 3 | **Pre-game модуль** (DotaTime()<0) — отдельный модуль, варианты: `safe_tower`, `aggressive_mid`, `jungle_pressure`. **Особенности 1v1:** руны в 0:00 не спавнятся; нейтралы почти не атакуют. | MEDIUM |
| 4 | `item_tango_single` — разделённое танго имеет другое имя; проверить поддержку | LOW |
| 5 | **5v5 полный матч** до конца (Pusher vs Ganker) | MEDIUM |
| 6 | **LLM item_build 5v5** | MEDIUM |

### 22.9 НЕ КОММИТИТЬ

`playstyle_radiant.lua` / `playstyle_dire.lua` — тест-конфиги (regen_lane / tp_fountain свап).
В репо хранится канон (ChatGPT_Pusher / ChatGPT_Ganker). Пушим пачкой по команде.
