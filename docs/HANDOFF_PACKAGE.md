...
что правило меняет поведение как заявлено. `ability_timing` (save_for_execute vs
on_cooldown) — правило уже в движке, проверяется сейчас. `tower_aggression` — правила
ещё НЕТ в движке, нужно написать (Codex/по команде).
