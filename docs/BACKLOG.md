...
**2. ЗАКРЫТО 28.08, формулировка была неверной.** Мёртвыми были ветки `M.Pregame`
(`pg-disengage`, `pg-uphill-freeze`, `pg-pos`, позиционирование, `pregameDuel`) — удалены с
флагом `PRECREEP_HOLD_AT_TOWER`, `AIBLaneDuel.Pregame` и хуком `ctx.pregameDuel`.
А `precreep-contact`/`precreep-trade` живут в `M.PreCreepStandoff` (`t <= 25`) и ДОСТИЖИМЫ:
в `8968270421` рядом срабатывает `precreep-space` (20 park / 18 hold / 8 space). Почему они
нули — НЕ УСТАНОВЛЕНО: у `gemini` нет лицензии `PreEngageAllowed`, но у `grok` она есть
(`hero_priority="always"`), и там тоже ноль. Разбирать по сторонам. Осталось: арматура
`phase == "pregame"` в `duelState` (единственный вызов теперь `post_horn`).
