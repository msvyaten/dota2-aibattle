...
**3.5 ОТКРЫТЫЙ ВОПРОС — ПОРОГ ЧУВСТВИТЕЛЬНОСТИ (следующий тест):**
0.90/0.20 = разница 0.7 (жирная). Реальные разные LLM на 1 промпт дадут ~0.35/0.65 (разница 0.3).
ScaleDesire линейный: множители 0.35→×0.70, 0.65→×1.30 (соотн. 1.86×) vs сегодня 0.20/0.90 (4.5×).
Сила эффекта падает >2×, НО push снежно-комит (4.5×→×50 по towerDmg). Узкий гэп может ещё дать сигнал
или утонуть в шуме. МЕТОДОЛОГИЯ: на узком гэпе сигнал≈шум → нужно N прогонов и распределение (не 1 игра).
ТЕСТ: push_desire 0.65 vs 0.35, остальное нейтрально/идентично, 3 прогона, метрика towerDmg. Если 3/3
расходится → продукт жив на LLM-вариативности. Если шумит → пол чувствительности, рычаг: нелинейный
ScaleDesire (круче в середине, усиливать мелкие различия промптов).
...
## 12. Текущая позиция
**Фаза 3 В РАБОТЕ** (ветка `phase-2-team-dials`). Фаза 2 DONE. 12-й диал `roshan_desire` добавлен (полная
схема ~12 рычагов). LLM-пайплайн доказан: промпт→ChatGPT5.5→JSON→конфиг→поведение, сигнал виден на
реальных значениях (towerDmg 7926:159, §11 3.4). closeout-фикс затюнен (FINISH_DEAD=1 +гейт 600s, 63→34 мин).
**СЛЕДУЮЩЕЕ:** тест порога чувствительности push 0.65 vs 0.35 ×3 прогона (§11 3.5).
LIVE сейчас = ChatGPT_Pusher(Rad)/ChatGPT_Ganker(Dire) — для след. теста сменить на push-A/B.

**ЗАДАЧИ НА ДОКУМЕНТАЦИЮ (завтра, в память — не в файлы):** (1) разница «старт тестов → сейчас»;
(2) момент про rules (дискретные переключатели, потолок=код, не ограничены списком); (3) карта
возможностей движка (что на OHA / что требует своего бота); (4) карточка агента (12 диалов + rules +
item_build + драфт из curated-пула + имя + реплики в чат). **TODO-полировка:** tower-avoid в laning
(боты горят под вышкой — фикс через гейт `GetNearbyTowers(900)` в интерливе добивания); затупы на линии.
