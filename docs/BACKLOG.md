...
**Объяснимо, не находки (~30):** бутылки нет ни разу (`bottle=` всегда −1) → мертвы шесть
бутылочных веток; ни одного uphill-события на милише (`uphill-*` = 0) → ренджевые ветки
корректно выключены, туда же `melee-pack-*`, `own-ranged-*`; `dbg-*`; счётчики, добавленные
ПОСЛЕ этих матчей (`ward-*`, `recovery-enemy-near`); ключи, собираемые конкатенацией
(`deny-cand-`, `state-`, `state-desire-`) — ограничение инструмента.

**1. Руны видны и не берутся НИКОГДА.** `rune-ground-truth` со `status=1` (AVAILABLE) — **144
раза**, статусы 0/1/2 все наблюдаются, значит подсистема наблюдения жива. При этом ноль по
`rune-pressure-*`, `rune-result`, `rune-transaction`, `rune-stage-override`, `recovery-rune`,
`power-rune-yield`. **Руна доступна 144 раза и ни разу не взята и не оспорена.**
`rune_control` — один из 12 модель-facing диалов, и его эффект ненаблюдаем в принципе.
Крупнейшая находка списка.
...
**3. Окно смерти открывается 30 раз и не делает НИЧЕГО.** `dw-active=30`, при этом `dw-farm`,
`dw-heal`, `dw-tower` — нули. ⚠️ Ранее я записал это как «окно открылось однажды» — неверно,
то было одно число из одного матча. Враг мёртв, окно открыто, действий нет.
