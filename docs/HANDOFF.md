...
Что добавлено:
- `creep-aggro-back`: раннее survival-действие до ластхита. Если бот недавно получил урон от крипов и HP ниже `creep_aggro_relief_hp` (дефолт `0.68`) или рядом большая пачка крипов, он отходит к безопасной стороне/своей башне.
